...
class MyGame(arcade.Window):
    def __init__(self):
        ...
        self.tile_map = None
        self.scale_factor = 1.0  # Будет вычислен в load_level
        self.gravity_scaled = GRAVITY
        self.dash_speed_scaled = DASH_SPEED
    ...
    def load_level(self, tmx_file="levelstart.tmx"):
        self.tile_map = arcade.load_tilemap(tmx_file, scaling=1)  # Загружаем без масштаба сначала
        ...
        # Спавн игрока (в не масштабированных координатах)
        ...
        # Вычисляем размеры карты (не масштабированные)
        map_width = self.tile_map.width * tw
        map_height = self.tile_map.height * th
        # Вычисляем scale_factor, чтобы вписать в экран
        scale_x = SCREEN_WIDTH / map_width
        scale_y = SCREEN_HEIGHT / map_height
        self.scale_factor = min(scale_x, scale_y)
        # Масштабируем константы физики
        self.gravity_scaled = GRAVITY * self.scale_factor
        self.dash_speed_scaled = DASH_SPEED * self.scale_factor
        ...
        # Масштабируем все спрайты уровня: размер и позиции (кроме player)
        all_lists = [
        ...
        # Масштабируем позицию player (но не scale, как попросил)
        self.player.center_x *= self.scale_factor
        self.player.center_y *= self.scale_factor
        # Настраиваем фрикцию (не масштабируется)
        for plat in self.platforms:
            plat.friction = 1.2
        self.player.friction = 0.4
        # Создаём physics_engine с масштабированной гравитацией
        self.physics_engine = arcade.PhysicsEnginePlatformer(
            self.player, self.platforms, self.gravity_scaled
        )
        # Теперь центрируем уже масштабированный уровень
        self.center_world()

    def center_world(self):
        # Размеры карты с учётом масштаба
        map_width = self.tile_map.width * self.tile_map.tile_width * self.scale_factor
        map_height = self.tile_map.height * self.tile_map.tile_height * self.scale_factor
        offset_x = (SCREEN_WIDTH - map_width) / 2
        offset_y = (SCREEN_HEIGHT - map_height) / 2
        all_lists = [
            self.platforms, self.death_list, self.bkg_list, self.bkg2_list,
            self.goal_list, self.player_list
        ]
        for sprite_list in all_lists:
            for spr in sprite_list:
    ...
    def on_update(self, delta_time):
        if self.dash_remaining_time > 0:
            self.player.change_x = self.dash_speed_scaled * self.dash_dir_x
            self.player.change_y = self.dash_speed_scaled * self.dash_dir_y
